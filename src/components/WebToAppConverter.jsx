...
import { Smartphone, Upload, Lock, Sparkles, Loader2, Image as ImageIcon, CheckCircle2 } from 'lucide-react';
...
export default function WebToAppConverter({ user, onOpenAuth }) {
  ...
  const [splashPreview, setSplashPreview] = useState('/default-splash.png');
  ...
  const plans = [
    {
      ...
      price: 0,
      buildType: 'apk',
      desc: '1 APK, Default Assets & Package'
    },
    {
      id: 'aab',
      name: 'Play Store AAB',
      price: 3,
      buildType: 'aab',
      desc: '1 AAB File + Custom Icon + Custom Splash'
    },
    {
      id: 'custom',
      name: 'Custom Package',
      price: 8,
      buildType: 'apk',
      desc: 'Custom Icon + Splash + Custom Package Name',
      badge: 'POPULAR'
    },
    {
      id: 'pro',
      ...
      price: 15,
      buildType: 'both',
      desc: 'APK + AAB + Full Assets + Versioning + No Ads + VIP Support'
    }
  ];

  const activePlan = plans.find(p => p.id === selectedPlan);

  // Exact Feature Permissions based on Plan
  const isIconAllowed = selectedPlan !== 'free'; // Allowed in $3, $8, $15
  const isSplashAllowed = selectedPlan !== 'free'; // Allowed in $3, $8, $15
  const isPackageAllowed = selectedPlan === 'custom' || selectedPlan === 'pro'; // Allowed in $8, $15
  const isVersionAllowed = selectedPlan === 'pro'; // Allowed ONLY in $15
        ...
        .insert([
          {
            ...
            version_code: isVersionAllowed ? versionCode : '1',
            version_name: isVersionAllowed ? versionName : '1.0.0',
            icon_url: finalIconUrl,
            splash_url: finalSplashUrl,
            no_ads: selectedPlan === 'pro',
            priority_support: selectedPlan === 'pro',
  ...
  return (
    <div className="max-w-md mx-auto p-2">
      <div className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-4 shadow-2xl">
        <form onSubmit={handleStartBuild} className="space-y-3.5">
          ...
          {/* 3. APP ICON */}
          <div className="border border-slate-800/60 rounded-xl p-2.5 bg-[#070a12]">
            <div className="flex items-center justify-between mb-1.5">
              ...
              {!isIconAllowed && (
                <span className="text-[8px] text-amber-400/90 bg-amber-500/10 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                  <Lock size={9} /> $3+ Plans Only
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-lg bg-slate-900 border border-slate-700 overflow-hidden flex items-center justify-center shrink-0">
                {iconFile ? (
                  ...
                  <span className="text-[7px] font-bold text-slate-500 text-center leading-tight">DEFAULT ICON</span>
                )}
              </div>

              <div className="flex-1">
                <input type="file" accept="image/*" ref={iconInputRef} onChange={handleIconChange} className="hidden" disabled={!isIconAllowed} />
                <button
                  type="button"
                  onClick={() => iconInputRef.current?.click()}
                  disabled={!isIconAllowed}
                  className={`w-full py-1.5 px-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1 border transition ${
                    !isIconAllowed
          ...
          <div className="border border-slate-800/60 rounded-xl p-2.5 bg-[#070a12]">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-12 rounded-lg bg-slate-900 border border-slate-700 overflow-hidden flex items-center justify-center shrink-0">
                {splashFile ? (
                  <img src={splashPreview} alt="Splash" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-[7px] font-bold text-slate-500 text-center leading-tight">DEFAULT SPLASH</span>
                )}
              </div>

              <div className="flex-1">
                <input type="file" accept="image/*" ref={splashInputRef} onChange={handleSplashChange} className="hidden" disabled={!isSplashAllowed} />
                <button
                  type="button"
                ...
                >
                  <Upload size={11} />
                  {splashFile ? 'Splash Selected' : 'Upload Splash'}
                </button>
              </div>
            </div>
          </div>
          ...
          {/* 5. SELECT BUILD PLAN */}
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5">5. SELECT BUILD PLAN</label>
              ...
              {plans.map((plan) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-slate-400 mt-0.5">{plan.desc}</p>
                  </div>
          ...
          {/* PACKAGE ID & VERSIONING OPTIONS (DYNAMIC BASED ON PLAN) */}
          {isPackageAllowed && (
            <div className="space-y-2 bg-[#070a12] p-2.5 rounded-xl border border-amber-500/30">
              <label className="block text-[9px] font-bold text-amber-400 uppercase">PACKAGE NAME & VERSION</label>
              <input
                type="text"
                value={packageName}
              ...
              {isVersionAllowed ? (
                ...
                <p className="text-[8px] text-slate-500">Version editing is unlocked in $15 Pro Plan</p>
              )}
            </div>
          )}

          {/* PRO PLAN PERKS BADGE */}
          {selectedPlan === 'pro' && (
            <div className="flex items-center justify-around bg-emerald-500/10 border border-emerald-500/30 p-2 rounded-xl text-[9px] text-emerald-400 font-bold">
              <span className="flex items-center gap-1"><CheckCircle2 size={10} /> NO ADS INCLUDED</span>
              <span className="flex items-center gap-1"><CheckCircle2 size={10} /> VIP HELP SUPPORT</span>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-3 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition shadow-xl disabled:opacity-50 mt-2"
          >
            {loading ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
              <>
                <Smartphone size={16} /> START BUILDING
              </>
            )}
          </button>
